...
*** Config-with-vlead-user
    Configures all other containers from ansible machine using vlead
    user.
**** Tasks/main.yml 
...
**** Copy-sshkeys of ansible/config-server to containesr
     ...
     including ansible/config-server
...
#+END_SRC
**** Set Proxy
     This script sets-up proxy in all containers by appending proxy
     line.
...
**** Dns Server
     Configures dns-servers by executing playbooks =public_dns.yaml= and
     =private_dns.yaml=
...
**** Comment some of the lines in systems-model roles
     Modifies all yaml files to create necessary environment for
     runnning site.yaml script
...
- name: commenting lab_rsnapshot_server.yaml in site.yaml
  replace:
     path: systems-model/build/{{clustername}}/site.yaml
     regexp: '^- include: lab_rsnapshot_server.yaml'
     replace: '#- include: lab_rsnapshot_server.yaml'
...
**** Set network proxy in server containers
     ...
     cluster's ansible container, this playbook will be running.
