...
- name: Configuring all server nodes
  shell: ansible-playbook -i hosts public_dns.yaml & ansible-playbook -i hosts private_dns.yaml & ansible-playbook -i hosts router.yaml & ansible-playbook -i hosts config_server.yaml & ansible-playbook -i hosts rsnapshot_server.yaml  &  & ansible-playbook -i hosts rsyslog_server.yaml & ansible-playbook -i hosts reverseproxy_server.yaml & ansible-playbook -i hosts nagios_server.yaml && sudo iptables -F
  register: out
  args:
     chdir: systems-model/build/{{clustername}}/
- debug: var=out.stdout_lines
