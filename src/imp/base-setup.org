...
*** Generate SSH-Keys on Host machine
     ...
     Public key of the host machine is required
 
     1. for running ansible-playbooks on localhost 
     2. to run playbooks on config-server.
     3. To clone systems-model repo from bitbucket.org
...
*** Include all tasks files 
    The following playbook contains included files. These included
    files are already described above. So that, all in one
    playbook. This playbooks sets up base machine. 
